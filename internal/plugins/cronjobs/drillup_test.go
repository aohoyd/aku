...
func cronWithOwners(name, namespace string, refs ...map[string]any) *unstructured.Unstructured {
...
func TestCronJobDrillUp(t *testing.T) {
	...
	t.Run("cronjob owned by operator CR resolves parent", func(t *testing.T) {
		...
		p := &Plugin{}
		cron := cronWithOwners("backup", "default", drillUpOwnerRef("example.com/v1", "Widget", "w", "widget-uid", true))

		gotPlugin, gotObj := p.DrillUp(cl, cron)
	...
	t.Run("ownerless cronjob is a no-op", func(t *testing.T) {
		store := k8s.NewStore(nil, "", nil)
		cl := plugintest.NewFakeClusterWithDiscovery(store, disc)

		p := &Plugin{}
		cron := cronWithOwners("backup", "default")
