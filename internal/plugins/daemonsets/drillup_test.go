...
func dsWithOwners(name, namespace string, refs ...map[string]any) *unstructured.Unstructured {
...
func TestDaemonSetDrillUp(t *testing.T) {
	...
	t.Run("daemonset owned by operator CR resolves parent", func(t *testing.T) {
		...
		p := &Plugin{}
		ds := dsWithOwners("agent", "default", drillUpOwnerRef("example.com/v1", "Widget", "w", "widget-uid", true))

		gotPlugin, gotObj := p.DrillUp(cl, ds)
	...
	t.Run("ownerless daemonset is a no-op", func(t *testing.T) {
		store := k8s.NewStore(nil, "", nil)
		cl := plugintest.NewFakeClusterWithDiscovery(store, disc)

		p := &Plugin{}
		ds := dsWithOwners("agent", "default")
