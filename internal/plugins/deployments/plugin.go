...
// in template containers by passing live ConfigMaps and Secrets from the
// focused cluster's store.
...
func (p *Plugin) DrillDown(cl plugin.Cluster, obj *unstructured.Unstructured) (plugin.ResourcePlugin, []*unstructured.Unstructured) {
	...
	store.Subscribe(replicasetsGVR, obj.GetNamespace())
	children := workload.FindOwned(store, replicasetsGVR, obj.GetNamespace(), string(obj.GetUID()))
