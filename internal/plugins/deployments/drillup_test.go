...
func deployWithOwners(name, namespace string, refs ...map[string]any) *unstructured.Unstructured {
...
func TestDeploymentDrillUp(t *testing.T) {
	...
	t.Run("deployment owned by operator CR resolves parent", func(t *testing.T) {
		...
		p := &Plugin{}
		deploy := deployWithOwners("web", "default", drillUpOwnerRef("example.com/v1", "Widget", "w", "widget-uid", true))

		gotPlugin, gotObj := p.DrillUp(cl, deploy)
	...
	t.Run("ownerless deployment is a no-op", func(t *testing.T) {
		store := k8s.NewStore(nil, "", nil)
		cl := plugintest.NewFakeClusterWithDiscovery(store, disc)

		p := &Plugin{}
		deploy := deployWithOwners("web", "default")
