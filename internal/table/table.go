...
type Model struct {
	...
	// styles.Selected apply (and only when active=true; an inactive cursor with
	// no override falls through to a plain row), while returning a non-nil style
	// overrides even the cursor highlight. The active argument carries the
	// table's live selectionActive flag so the callback need not capture focus
	// state. See renderRow for the exact behavior.
	RowStyleFunc func(index int, isCursor, active bool) *lipgloss.Style
...
type Styles struct {
	...
	Cell   lipgloss.Style
	// Selected is the cursor-row fill, applied by renderRow only when
	// selectionActive is true. It is no longer swapped to a dim style on blur;
	// do not reintroduce a dim-swap to hide the cursor — use
	// SetSelectionActive(false) instead.
...
// SelectionActive reports whether the cursor row is currently shown as the
// active selection (see SetSelectionActive). This is the single source of
// truth for the selection-active state; callers should not mirror it.
func (m Model) SelectionActive() bool { return m.selectionActive }

// SetSelectionActive sets whether the cursor row is shown as the active
// selection. It is a no-op when the value is unchanged; otherwise it records
// the flag and re-renders so renderRow's cursor decision reflects it. Distinct
// from Focus/Blur, which gate keyboard input.
func (m *Model) SetSelectionActive(b bool) {
	if m.selectionActive == b {
		return
	}
	m.selectionActive = b
	m.UpdateViewport()
...
func (m *Model) renderRow(r int) string {
	...
	if r == m.cursor {
		if m.RowStyleFunc != nil {
			if override := m.RowStyleFunc(r, true, m.selectionActive); override != nil {
		...
		// inactive cursor, no override → fall through to a plain row
	}
	if m.RowStyleFunc != nil {
		if override := m.RowStyleFunc(r, false, m.selectionActive); override != nil {
			return override.Render(stripStyleKeepReverse(row))
		}
	}
	return row
